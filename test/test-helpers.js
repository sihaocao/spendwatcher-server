      ...
      trx
        ...
        .then(() =>
          Promise.all([
            ...
            trx.raw(
              `ALTER SEQUENCE spendwatcher_users_id_seq minvalue 0 START WITH 1`
            ),
            trx.raw(`SELECT setval('expense_logs_id_seq', 0)`),
            trx.raw(`SELECT setval('spendwatcher_users_id_seq', 0)`),
          ])
        )
    );
}
...
function seedUsers(db, users) {
    ...
    return db
      .into('spendwatcher_users')
      .insert(preppedUsers)
      .then(() =>
        // update the auto sequence to stay in sync
        db.raw(`SELECT setval('spendwatcher_users_id_seq', ?)`, [
